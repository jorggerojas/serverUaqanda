...
function obtenerViajes($inicial, $final){
    ...
    $datos = $db->query("SELECT v.idViaje AS idViaje, r.PuntoInicial, r.PuntoFinal, r.Adicional, v.Horario, u.Capacidad
                          ...
                          v.Status != 'Eliminado' OR
                          u.idUnidad = v.idUnidad AND
                          v.idRuta = r.idRuta AND
                          r.PuntoInicial = '{$inicial}' AND r.PuntoFinal = '{$final}' AND fecha = '$hoy'AND v.Status != 'En curso'
...
function historialViajes($user){
  ...
  $r = $db->query("SELECT DISTINCT res.idViaje as 'id', v.Fecha as 'Fecha', v.Horario as 'Hora', r.PuntoInicial as 'Inicio', r.PuntoFinal as 'Final'
                  ...
                  v.idRuta = r.idRuta AND res.Status != 'Reservado' OR res.idUsuario ='$user' AND res.idViaje = v.idViaje AND
                  v.idRuta = r.idRuta AND res.Status = 'Finalizado'  LIMIT 6");
  if(is_array($r)){
    $temp = $r;
  }else{
    $temp = false;
...
function dropViaje($us, $id){
  $db = new Conexion();
  $delete = $db->delete("DELETE FROM reservaciones WHERE idUsuario = '$us' AND idViaje = '$id' AND Status != 'Confirmado'
                          OR idUsuario = '$us' AND idViaje = '$id' AND Status != 'En curso'");
  if($delete){
    return true;
  }else{
    return false;
